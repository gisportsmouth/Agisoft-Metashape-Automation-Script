...
class PS_Proc(object):
    ...
    def iterate_grad(self, chunk, filter, step, 
                        total_ties, sel_value, threshold):
        """
        ...
        Input: chunk - the chunk to be processed    
                ...
                total_ties - the starting value tie points after align
        ...
        hard_limit = total_ties * self.tp_pcnt
        while True:
            ...
            pcent_total = 100 * sel / total_ties
    ...
    def grad_sel_preGCP(self, *, rec_uncert=None,
        ...
        for _ in self.chunks:
            ...
            self.total_points = len([i for i in _.point_cloud.points])
            target_points = self.total_points * self.tp_pcnt
            #1 RecUncert
            ...
            _.optimizeCameras(adaptive_fitting=adapt)  
            #write log information
            removed = self.total_points - len([i for i in _.point_cloud.points])
            pcent = 100 * removed / self.total_points
            with open(self.log, 'a+') as logfile:
                end_t = datetime.now()
                ...
                logfile.write('    Aim to reduce to (%): {} \n'\
                                '    Adaptive fitting (%): {} \n'.
                                            format(self.tp_pcnt,
                                                    adapt))
            f = PhotoScan.PointCloud.Filter()
            #in case preGCP grad ws not run
