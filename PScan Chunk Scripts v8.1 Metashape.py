...
Change log:
    ...
    v8.0
        ...
        updated scrpt to reflect name changes in 2.0 API          
    v7.8
        changed align option for run_model to generic=True
        added parameter to align for guided matching default True
        added align option guided_matching=guided is good for vegetation, grass, trees etc., as well as large sensors
...
With GCP or scales or if wanting to check alignment:
    ...
    is better. If markers exist you can import them and run the whole process to 
    save time (see above).
...
if found_major_version != compatible_major_version:
    raise Exception('Incompatible Metashape version: '
                    f'{found_major_version} != {compatible_major_version}')
...
class MS_Proc(object):
    ...
    def __init__(self, doc, *, 
        ...
        """
        self.path = Path(Metashape.app.getExistingDirectory(
        ...
        docname = Path(str(self.doc).strip("<,>,', Document")).stem
        self.log = (self.path / 
                    f'{docname}'
    ...
    def info(self):
        ...
        print(f'The current path is: {self.path}')
    ...
    def run_custom(self):
        ...
        min_qual = Metashape.app.getFloat(
                        label='Enter picture quality threshold', value=0.5)
    ...
    def run_qual_adjust(self):
        ...
        match_acc = Metashape.app.getInt(
                        label='Enter image matching accuracy '
                                '(Highest=0 High=1 Medium=2 Low=4 Lowest=8)', 
                        value=1)
        if match_acc in self.match_dict:
            self.match_acc = match_acc
    ...
    def iterate_grad(self, 
        ...
        Input: chunk - the chunk to be processed    
                filter - the Metashape.TiePoints.Filter object
        ...
        while True:
            ...
            if (remain_ties > hard_limit 
                ...
                adjust += step
                print(f'Adjusted by {round(adjust, 2)} to '
    ...
    def grad_sel_postGCP(self, *, repro_error=0,
        ...
        for _ in self.chunks:
            ...
            with open(self.log, 'a+') as logfile:
                start_t =  datetime.now()
                logfile.write(f"Running postGCP grad selection of {_} at "
                                f"{start_t.strftime('%H:%M:%S')} \n")
    ...
    def dense_c(self, *, mode=None, qual=None):
        ...
        for _ in self.chunks:
            ...
            _.buildDepthMaps(downscale=qual, 
                                filter_mode=mode)            
            _.buildPointCloud(point_colors=True,
    ...
    def build_model(self, *, surf=Metashape.Arbitrary,
        ...
        for _ in self.chunks:
            ...
            try:
                ...
                with open(self.log, 'a+') as logfile:
                    start_t =  datetime.now()
                    logfile.write(f'***A memory error occurred in chunk: {_}***')
                continue
        ...
        for _ in self.chunks:
            ...
            with open(self.log, 'a+') as logfile:
                ...
                try:
                    ...
                    _.exportRaster(path=str(self.path / file), 
        ...
        for _ in self.chunks:
            ...
            with open(self.log, 'a+') as logfile:
                ...
                try:
                    ...
                    _.exportPointCloud(path=str(self.path / file),
                    ...
                    if str(e) == 'Null point cloud':
                        t = (f'There is no point cloud to export in chunk: {_}\n')
                        print(t)
                        logfile.write(t)
                    else:
                        raise
                try:
                    ...
                    _.exportModel(path=str(self.path / file),
                                    texture_format=ext,
                                    crs=crs)
                    t = f'File: {file}\n'
                    print(t)
                ...
                try:
                    ...
                    _.exportReport(path=str(self.path / file),
    ...
    def blue_flag():
        ...
        for marker in chunk.markers:
            ...
            for camera in [c for c in chunk.cameras 
                            if c.transform 
                            and c.type == Metashape.Camera.Type.Regular]:
    ...
    def menu_geo_grad(self):
        ...
        with open(self.log, 'a+') as logfile:
            start_t =  datetime.now()
            logfile.write('Menu item: Geo (gradual selection)/n')
