...
With GCP or scales or if wanting to check alignment:
    Having the images aligned helps with marker placement. If markers are placed already there is no
    quality difference as to whether you align on image coords first and then switch to GCP, 
    although some sources claim this is better. If markers exist you can run the whole process to 
    save time (see above).
...
class MS_Proc(object):
    ...
    def __init__(self, doc, *, 
        ...
        if match_acc in self.match_dict:
            ...
            raise ValueError('Unknown Metashape matching accuracy value (Highest=0 High=1 Medium=2 Low=4 Lowest=8).')
        if depth_qual in self.depth_dict:
            self.depth_qual = depth_qual
        else:
            raise ValueError('Unknown Metashape depth map quality value (Ultra=1 High=2 Medium=4 Low=8 Lowest=16).')
        self.total_points = {}
        self.exp_crs = 0
    ...
    def info(self):
        ...
        print('The current matching accuracy is: {}'.format(self.match_dict[self.match_acc]))
        print('The current depth map quality is: {}'.format(self.depth_dict[self.depth_qual]))
    ...
    def run_qual_adjust(self):
        ...
        Change the object attributes for:
                    ...
                    The depth map quality
        """
        match_acc = Metashape.app.getInt(
                        label='Enter image matching accuracy (Highest=0 High=1 Medium=2 Low=4 Lowest=8)', 
                        value=1)
        if match_acc in self.match_dict:
            self.match_acc = match_acc
        else:
            raise ValueError('Unknown Metashape matching accuracy value (Highest=0 High=1 Medium=2 Low=4 Lowest=8).')
        depth_qual = Metashape.app.getInt(
                        label=('Enter depth map quality (Ultra=1 High=2 Medium=4 Low=8 Lowest=16)'), 
                        value=2)
        if depth_qual in self.depth_dict:
            self.depth_qual = depth_qual
        else:
        ...
        for _ in self.chunks:
            for camera in _.cameras:
                if camera.reference.location:
                    ...
                    camera.reference.location = Metashape.Vector([coord.x, coord.y, coord.z * -1])
    ...
    def dense_c(self, *, mode=None, qual=None):
        ...
        for _ in self.chunks:
            ...
            _.buildDepthMaps(downscale=qual, 
                                filter_mode=mode)            
            ...
            with open(self.log, 'a+') as logfile:
                ...
                logfile.write('Building DEM {} at {} \n'.
                                            format(_, 
                                                start_t.strftime('%H:%M:%S')))
            #build DEM                            
            _.buildDem(source_data=Metashape.DenseCloudData, 
            ...
            with open(self.log, 'a+') as logfile:
                ...
                try:
                    ...
                    if str(e) == 'Null point cloud':
                        t = ('ERROR: There is no point cloud to export in chunk: {}\n'
                                .format(_))
                        print(t)
                        logfile.write(t)
                        continue
                ...
                try:
                    ...
                    _.exportRaster(path = os.path.join(self.path, file), 
                    ...
                    if str(e) == 'Null elevation':
                        t = ('ERROR: There is no elevation to export in chunk: {}\n'
                                .format(_))
                        print(t)
                        logfile.write(t)
                        continue
                ...
                #Export Ortho
                    ...
                    if str(e) == 'Null orthomosaic':
                        t = 'ERROR: There is no orthomosaic to export in chunk: {}\n'.format(_)
                        print(t)
                        logfile.write(t)
                        continue
                        ...
                        try:
                            ...
                            print(t)
                            logfile.write(t)
                            t = 'WARNING: TIFF is too large, esported as BigTIFF\n'
                            print(t)
                            logfile.write(t)
                            continue
                        except:
                            t = 'ERROR: TIFF is too large to export as a single chunk, export manually\n'
                            print(t)
                            logfile.write(t)
                            continue
                    else:
        ...
        for _ in self.chunks:
            ...
            with open(self.log, 'a+') as logfile:
                ...
                try:
                    ...
                    if str(e) == 'Null point cloud':
                        t = ('There is no point cloud to export in chunk: {}'
                                .format(_))
                        print(t)
                        logfile.write(t)
                        continue
                ...
                try:
                    ...
                    if str(e) == 'Null model':
                        t = 'There is no model to export in chunk: {}'.format(_)
                        print(t)
                        logfile.write(t)
                        continue
                    else:
    ...
    def menu_geo_post_align_grad(self):
        ...
        with open(self.log, 'a+') as logfile:
            start_t =  datetime.now()
            logfile.write('Menu item: Run all after alignment - Geo (gradual selection)')
    ...
    def menu_model_grad_mask_hi(self):
        ...
        with open(self.log, 'a+') as logfile:
            start_t =  datetime.now()
            logfile.write('Menu item: 3D Model (gradual selection, mask ties, high accuracy)')
...
menu('Custom/Run - 3D Model (grad, mask ties, high accuracy)', ms_doc.menu_model_grad_mask_hi)
